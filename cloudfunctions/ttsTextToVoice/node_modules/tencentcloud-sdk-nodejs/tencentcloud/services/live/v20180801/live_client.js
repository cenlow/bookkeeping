...
查询某段时间top n客户端ip汇总信息（暂支持top 1000）
     ...
     * 创建关键词库，直播审核功能使用。
     */
    async CreateAuditKeywordLib(req, cb) {
...
注意：至少填写一个回调 URL。
     ...
     * 删除关键词信息。
     */
    async DeleteAuditKeywords(req, cb) {
...
查询按省份和运营商分组的下行播放数据。
     ...
     * 查询拉流转推任务流数据统计信息。
     */
    async DescribePullTransformPushInfoList(req, cb) {
...
推流计费会先取全球推流用量和全球播放用量进行比较，满足计费条件后再按各地区用量出账。详情参见[计费文档](https://cloud.tencent.com/document/product/267/34175)。
     ...
     * 用于关闭回源客户源站功能
     */
    async CloseSourceStream(req, cb) {
...
查询实时的域名维度下行播放数据，由于数据处理有耗时，接口默认查询4分钟前的准实时数据。
     ...
     * 创建关键词，并关联到关键词库。
     */
    async CreateAuditKeywords(req, cb) {
...
注意：单个域名仅支持关联一个截图模板。
     ...
     * 获取关键词信息。
     */
    async DescribeAuditKeywords(req, cb) {
...
注意：云端特效生成需要一定时间，调用生成接口后，可通过查询接口，获取是否特效已生成。
     */
    async CreateLiveCloudEffect(req, cb) {
