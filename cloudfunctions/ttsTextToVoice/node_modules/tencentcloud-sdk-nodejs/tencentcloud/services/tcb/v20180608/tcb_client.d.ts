...
返回的 KeyID 和 PrivateKey 需与环境 ID 一起组装为 JSON 配置文件，供客户端 Admin SDK 初始化时使用，文件格式如下：
{
  ...
  "env_id": "<请求时传入的 EnvId>"
     ...
     */
    CreateCustomLoginKey(req: CreateCustomLoginKeyRequest, cb?: (error: string, rep: CreateCustomLoginKeyResponse) => void): Promise<CreateCustomLoginKeyResponse>;
    /**
     * 查询tcb用户列表
     */
...
安全规则，用于控制C端用户的访问权限。详见 [安全规则介绍 ](https://cloud.tencent.com/document/product/876/123478)。
     */
    ModifySafeRule(req: ModifySafeRuleRequest, cb?: (error: string, rep: ModifySafeRuleResponse) => void): Promise<ModifySafeRuleResponse>;
    /**
     * 删除合法域名。
云开发会校验网页应用请求的来源域名，您需要将来源域名加入到WEB安全域名列表中。
